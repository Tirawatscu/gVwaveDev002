# /*****************************************************************************
# * | File        :   ADS1263.py
# * | Author      :   Tirawat Boonyatee
# * | Function    :   ADS1263 driver for ESP8266
# * | Info        :
# *----------------
# * | This version:   V1.0
# * | Date        :   2023-04-30
# * | Info        :   
# ******************************************************************************/
from config32 import config

# Initialize the module
my_config = config()
my_config.module_init()

# Now you can call other methods
my_config.set_RST(1)
my_config.set_CS(0)
my_config.delay_ms(10)
...
class ADS1263:
    ...
    def ADS1263_WriteCmd(self, reg):
        my_config.set_CS(0)#cs  0
        my_config.spi_writebyte([reg])
        my_config.set_CS(1)#cs 1
    
    
    def ADS1263_WriteReg(self, reg, data):
        my_config.set_CS(0)#cs  0
        my_config.spi_writebyte([ADS1263_CMD['CMD_WREG'] | reg, 0x00, data])
        my_config.set_CS(1)#cs 1
        
        
    def ADS1263_ReadData(self, reg):
        my_config.set_CS(0)#cs  0
        my_config.spi_writebyte([ADS1263_CMD['CMD_RREG'] | reg, 0x00])
        data = my_config.spi_readbytes(1)
        my_config.set_CS(1)#cs 1
        return data
    ...
    def ADS1263_init_ADC1(self, Rate1=None):
        if Rate1 is None:
            Rate1 = 'ADS1263_14400SPS'
        self.ADS1263_reset()
    ...
    def ADS1263_Read_ADC_Data(self):
        my_config.set_CS(0)#cs  0
        while(1):
            my_config.spi_writebyte([ADS1263_CMD['CMD_RDATA1']])
            # my_config.delay_ms(10)
            if(my_config.spi_readbytes(1)[0] & 0x40 != 0):
                break
        buf = my_config.spi_readbytes(5)
        my_config.set_CS(1)#cs 1
